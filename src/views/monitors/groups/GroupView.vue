<script lang="ts" setup>
    ...
    async function deleteGroup (id: string | number) {
        await dropDowns().destroy('groups',id,'groups').then(() => {
        })
    }
    ...
    async function fetchData () {
        await dropDowns().get('groups').then((response) => {
...
<template>
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        ...
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <button class="btn btn-primary shadow-md mr-2" @click="create">Crear una grupo</button>
        </div>
    </div>
    ...
    <div class="intro-y box p-5 mt-5">
        <BaseCrud :headers="headers" :items="items" label="La grupo" :on-delete-fnc="deleteGroup" />
    </div>
</template>
