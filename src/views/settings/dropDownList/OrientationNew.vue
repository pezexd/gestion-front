...
const fetchOne = async () => {
	return await orientations.getOne('orientations', route.params.id)
}
...
onMounted(async () => {
	if (route.params.id) {
		...
		const { id, created_at, updated_at, ...rest } = orientations.dataOne.value
		form.value = { ...rest }
	} else {
		drop_downs.$reset()
	}
...
const onSubmit = async () => {
	...
	if (valid){
		if (isEditing.value)
			await orientations.update('orientations', orientations.dataOne.value.id, drop_downs.transpiledData, 'orientations')
		else
			await orientations.create('orientations', drop_downs.transpiledData, 'orientations')
	}
	else {
		alerts.validation()
	}
...
<template>
	<div class="intro-y flex flex-col items-start gap-1 mt-8">
		...
		<h2 v-if="isEditing" class="text-lg font-medium mr-auto">
			Edición de orientación: <b>{{ orientations.dataOne.value.id }}</b>
		</h2>
		<h2 v-else class="text-lg font-medium mr-auto">
			Ingresar orientación
		</h2>
	</div>
	<div class="intro-y box mt-5">
		<form @submit.prevent="onSubmit"
			...
			<section id="GeneralData"
				...
				<div class="w-full">
					<BaseInput type="text" label="NOMBRE *" tooltip="Ingrese el nombre de la orientación"
						placeholder="Orientación..." name="name" v-model="form.name" :validator="v$" />
				</div>
			</section>
