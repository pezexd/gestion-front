<script lang="ts" setup>
    ...
    async function deleteExpertice (id: string | number) {
        await dropDowns().destroy('expertises',id,'expertises').then(() => {
        })
    }
    ...
    async function fetchData () {
        await dropDowns().get('expertises').then((response) => {
...
<template>
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Experticia</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <button class="btn btn-primary shadow-md mr-2" @click="create">Crear una experticia</button>
        </div>
    </div>
    ...
    <div class="intro-y box p-5 mt-5">
        <BaseCrud :headers="headers" :items="items" label="La experticia" :on-delete-fnc="deleteExpertice" />
    </div>
</template>
