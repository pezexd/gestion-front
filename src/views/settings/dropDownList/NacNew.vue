...
const fetchOne = async () => {
	return await nacs.getOne('nacs', route.params.id)
}
...
onMounted(async () => {
	if (route.params.id) {
		...
		const { id, created_at, updated_at, ...rest } = nacs.dataOne.value
		form.value = { ...rest }
	} else {
		drop_downs.$reset()
	}
...
const onSubmit = async () => {
	...
	if (valid){
		if (isEditing.value)
			await nacs.update('nacs', nacs.dataOne.value.id, drop_downs.transpiledData, 'nacs')
		else
			await nacs.create('nacs', drop_downs.transpiledData, 'nacs')
	}
	else {
		alerts.validation()
	}
...
<template>
	<div class="intro-y flex flex-col items-start gap-1 mt-8">
		...
		<h2 v-if="isEditing" class="text-lg font-medium mr-auto">
			Edición de nac: <b>{{ nacs.dataOne.value.id }}</b>
		</h2>
	...
	<div class="intro-y box mt-5">
		<form @submit.prevent="onSubmit"
			...
			<section id="GeneralData"
				...
				<div class="w-full">
					<BaseInput type="text" label="NOMBRE *" tooltip="Ingrese el nombre del NAC"
						placeholder="NAC..." name="name" v-model="form.name" :validator="v$" />
				</div>
			</section>
