<script lang="ts" setup>
    ...
    async function deleteNac (id: string | number) {
        await dropDowns().destroy('nacs',id,'nacs').then(() => {
        })
    }
    ...
    async function fetchData () {
        await dropDowns().get('nacs').then((response) => {
...
<template>
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">NAC</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <button class="btn btn-primary shadow-md mr-2" @click="create">Crear un NAC</button>
        </div>
    </div>
    ...
    <div class="intro-y box p-5 mt-5">
        <BaseCrud :headers="headers" :items="items" label="El nac" :on-delete-fnc="deleteNac" />
    </div>
</template>
