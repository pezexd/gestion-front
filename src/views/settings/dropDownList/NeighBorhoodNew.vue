...
const fetchOne = async () => {
	return await neighborhoods.getOne('neighborhoods', route.params.id)
}
...
onMounted(async () => {
	if (route.params.id) {
		...
		const { id, created_at, updated_at, ...rest } = neighborhoods.dataOne.value
		form.value = { ...rest }
	} else {
		drop_downs.$reset()
	}
...
const onSubmit = async () => {
	...
	if (valid){
		if (isEditing.value)
			await neighborhoods.update('neighborhoods', neighborhoods.dataOne.value.id, drop_downs.transpiledData, 'neighborhoods')
		else
			await neighborhoods.create('neighborhoods', drop_downs.transpiledData, 'neighborhoods')
	}
	else {
		alerts.validation()
	}
...
<template>
	<div class="intro-y flex flex-col items-start gap-1 mt-8">
		...
		<h2 v-if="isEditing" class="text-lg font-medium mr-auto">
			Edición de barrio: <b>{{ neighborhoods.dataOne.value.id }}</b>
		</h2>
