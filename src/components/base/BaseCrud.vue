...
const get_edit = (item: { user_id: string | number, created_by: string | number, status: statuses }) => {
    ...
    const is_equals = created_by == logged_id || user_id == logged_id || created_by?.id === logged_id;
...
<template>
    <div class="intro-y flex flex-col gap-2">
        ...
        <DataTable :headers="headers" :items="items" :sort-by="sorts.by" :filter-options="filter_options"
            :search-field="filters.search_field" :search-value="filters.search_value" :sort-type="sorts.type"
            buttons-pagination table-class-name="customize-table">
            ...
            <template #item-start_time="{ start_time }">
                <div class="customize-item">
                    <p>{{ (start_time != null && start_time != '') ? dayjs(`2000-01-01 ${start_time}`).format("hh:mm") : '' }}</p>
                </div>
            </template>
            <template #item-final_hour="{ final_hour }">
                <div class="customize-item">
                    <p>{{ (final_hour != null && final_hour != '') ? dayjs(`2000-01-01 ${final_hour}`).format("hh:mm") : '' }}</p>
                </div>
            </template>
            ...
            <template #item-actions="item">
                <div class="flex gap-2 justify-end">
                    ...
                    <template v-if="management_permissions && (item.status == 'ENREV' || item.status == 'REV')">
